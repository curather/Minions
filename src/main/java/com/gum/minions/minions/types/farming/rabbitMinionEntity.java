...
public class rabbitMinionEntity extends CombatMinionEntity {
    /*
    CUSTOMIZABLE MINION ATTRIBUTES
    get Textures and ids from https://minecraft-heads.com/
     */
    ItemStack playerHead = MinionSkins.RABBIT_MINION.playerHead();
    int armorColor = DyeColor.LIGHT_GRAY.getColorValue();
    EntityType setEntity = EntityType.RABBIT;
    HashMap<Item, Float> bonusItemList = new HashMap<>();
    Item retItem = Items.RABBIT;
    int minionSpeed = 26 * 20;
    Item MinionItem = ItemRegister.RABBIT_MINION;
    /*
    DON'T EDIT BELOW HERE
     */
    static rabbitMinionEntity armorstandentity;

    public rabbitMinionEntity(EntityType<? extends ArmorStandEntity> p_i50225_1_, World p_i50225_2_) {
        super(p_i50225_1_, p_i50225_2_);
        this.setMinionType(setEntity);
        this.setReturnItem(retItem);
        this.setSpeed(minionSpeed);
        bonusItemList.put(Items.RABBIT_FOOT, 35f);
        bonusItemList.put(Items.RABBIT_HIDE, 35f);
